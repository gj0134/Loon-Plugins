...
- 直播间横屏、竖屏活动广告弹层（直接关闭原生弹层，不保留空白容器）
...
- 关注时间线、热门信息流中的品牌官方推荐广告
- 荐读、粉丝头条等信息流广告
- 发现页会员横幅和“热门视频”推荐区
- 微博会员推广开关及“我的”页面会员入口
- 启动预加载、实时广告和开屏图片广告

信息流广告来自独立的 `/2/ad/weibointl` 接口。插件清空该接口返回的广告列表，并关闭国际版配置中的广告、会员推广和热门视频推荐开关，同时移除发现页会员横幅；不按品牌名或正文关键词过滤，也不修改正常关注时间线、热门微博正文、长微博、收藏和分组接口。
...
- 每次真实请求默认通知“✅ 运行正常”或“❌ 运行失败”；可在插件参数中关闭正常状态通知，失败和实际返款仍会提醒。
- 在 Loon 脚本页面运行“查看最近3次价保记录”，顶部会显示当前健康状态，并列出最近 3 次真实申请的时间和结果；超过 3 小时没有新记录会明确提示运行失败。
